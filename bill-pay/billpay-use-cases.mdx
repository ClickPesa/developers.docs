...
- Webshop generates the **Order BillPay Control Number**:
  - Can be **offline concatenation** or **API-generated** for more control (set amount, payment mode, max total, custom reference, description).
...
- API generation allows specifying:
  ...
  - A max total that can be collected (optional).
...
- Numbers can be assigned:
  ...
  - **Via API** (full flexibility):
    ...
    - Optionally set a **max total** so collections stop when the limit is reached.
