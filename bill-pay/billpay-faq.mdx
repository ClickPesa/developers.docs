...
- **API Generation:** Provides flexibility to specify:
  ...
  - Max total that can be collected (optional — closes the number when the limit is reached)
  - Bill description
...
- If you also set a **max total** (`billMaxCollectionAmount`), the payment must fit within what’s left to collect.
---

[Can I limit how much can be collected on a control number?](#)

**A:** Yes. When payment mode is `ALLOW_PARTIAL_AND_OVER_PAYMENT`, set **`billMaxCollectionAmount`** on create or update.

- Each payment counts toward the max. A payment that would go over what’s left is rejected.
- When the max is reached, the control number becomes inactive and stops accepting payments.
- You can set a max with or without `billAmount`. If you set both, the max must be at least as large as the bill amount.
- If a payment is reversed, that amount still counts toward the max.
- Works best with **customer** control numbers if you expect several payments toward the same limit. **Order** control numbers still accept only one payment.

See the [BillPay API](/bill-pay/billpay-api-overview) and [create customer control number](/api-reference/collection/billpay/create-customer-control-number).
...
- **Customer BillPay Control Number** – **Yes**
  ...
  - If you set a **max total**, further payments stop once that limit is reached.
...
- **No expiration by default:** Unlike Order BillPay Control Numbers, Customer BillPay Control Numbers do **not expire** and do **not close automatically** after each payment.
- **Exception — max total:** If you set `billMaxCollectionAmount`, the number becomes inactive when that limit is reached.
...
- Set a **max total** that can be collected (`billMaxCollectionAmount`), when using partial and over payments
