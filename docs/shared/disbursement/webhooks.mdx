...
> **Note:** These webhook events are applicable for all payouts initiated from any platform associated with ClickPesa — including the Merchant Dashboard, Hosted Integration, API, Embedded, and more. For example, the `PAYOUT INITIATED` webhook added will be triggered is triggered by payouts from all supported platforms.
