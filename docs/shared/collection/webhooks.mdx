...
> **Note:** These webhook events are applicable for all payments initiated from any platform associated with ClickPesa — including the Merchant Dashboard, Hosted Integration, API, Embedded, and more. For example, the `PAYMENT RECEIVED` event is triggered by payments from all supported platforms.
