...
- **[Verify Bank Account](/api-reference/disbursement/bank-payout/verify-bank-account)** resolves the beneficiary account holder name before you create a bank payout
- For CRDB accounts (`bic` `CORUTZTZ`), lookup uses the CRDB account name check and returns `accountName`
- Use the returned `accountName` in **[Create Bank Payout](/api-reference/disbursement/bank-payout/create-bank-payout)**
