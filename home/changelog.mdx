...
- **Application webhooks for BillPay and Collection API:** Payments against API-created BillPay control numbers and Collection API requests (including M-Pesa USSD push) now persist application context so `PAYMENT RECEIVED` and `PAYMENT FAILED` route to the correct [application webhook](/home/webhooks#application-level-webhooks) when configured.
...
- **Per application**: When configured, webhook events for transactions tied to that application (Collection API, Disbursement API, BillPay API, and hosted checkout or payout links created with the application's Client ID) are sent to the **application-level webhook**; merchant webhooks are not used in that flow
