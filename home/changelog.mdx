...
<Update label="September 2026" tags={["Improvements", "Merchant Dashboard", "Collection", "Payout"]} rss={{ title: "Monthly Updates", description: "September 2026 - Product updates and improvements" }}>
...
- Updated in the [Merchant Dashboard](https://merchant.clickpesa.com) and in [BillPay payment instructions](/bill-pay/bill-pay-overview#making-payments-with-billpay-control-numbers)
...
- **A support contact customers trust:** Set an email and phone under [Settings → General](/dashboard/settings/general). Customers see it on checkout, receipts, invoices, and payment confirmations — not your login address
...
- **Set it up:** **Settings → Notifications** — language, who gets alerts, and which activities (collections, disbursements, deposits). **Settings → General** — customer support. Full steps: [Notifications](/dashboard/notifications)

</Update>

<Update label="August 2026" tags={["New Feature", "Improvements", "API", "Payout", "Merchant Dashboard", "Collection", "Community"]} rss={{ title: "Monthly Updates", description: "August 2026 - Product updates and improvements" }}>

## Payment Pages: Fixed or open amount

Payment Pages can now collect a set price or let the payer choose how much to send:

- **Fixed amount:** Set the amount and currency. Use this for tickets, products, or any set price. **Allow Amount Update** in [Collection settings](/dashboard/settings/collection) still controls whether the payer can change that amount
...
Existing payment pages stay **fixed**. See [Payment Pages](/dashboard/payment-pages).

## Lipa Namba payouts on the Merchant Dashboard

You can now send Lipa Namba payouts from the [Merchant Dashboard](https://merchant.clickpesa.com), alongside bank and mobile money.

- **Send Via Lipa Namba:** On [Single Payout](/dashboard/disbursement/single-payout), choose Lipa Namba, pick a provider, enter the number, and click **Verify Name**. The beneficiary name is required before you can continue
...
- Added **Node.js SDK** by Deogratius Denis Mbombwe to the [Community Plugins](/sdk-plugins/plugins-and-extensions) section
...
- Available on npm as [`clickpesa-nodejs-sdk`](https://www.npmjs.com/package/clickpesa-nodejs-sdk)
...
BillPay custom **`billReference`** is also limited to **20 characters**. See [create order control number](/api-reference/collection/billpay/create-order-control-number).
