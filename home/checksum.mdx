...
<Note>
  ...
  - **Important**: When generating a checksum, ensure the payload does **not** include the `checksum` or `checksumMethod` fields. These fields should be excluded from the payload before checksum calculation.
...
2. **Extract Checksum Method** - If present, extract the `checksumMethod` field from the payload to determine which checksum algorithm to use:
   - `canonical` (default) - Uses the new recursive canonicalization method
   - `legacy` - Uses the legacy string concatenation method for backward compatibility

3. **Prepare Payload** - Before recomputing the checksum, **exclude** the following fields from the payload:
   - `checksum` - The checksum field itself
   - `checksumMethod` - The checksum method indicator (extract before excluding)

4. **Recompute Checksum** - Using the same `createPayloadChecksum` function, recompute the checksum from the prepared payload (without `checksum` and `checksumMethod` fields) using the checksum key and the extracted `checksumMethod` as an option.

5. **Compare the Computed and Received Checksum**
