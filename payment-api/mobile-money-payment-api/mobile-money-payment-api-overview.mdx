...
There's no API to create an [BillPay Control Numbers](/bill-pay/bill-pay-overview). You generate it by combining your Merchant BillPay-Namba with an reference. This full order number is shared with the customer as the payment reference. Once paid, ClickPesa routes the payment using the BillPay-Namba and sends a webhook with the full control number and payment details.